...
- **5 agentes de riesgo** (`server/agents/{financial,climate,soil,news,yield}/`), cada uno con mock fixture + scoring puro + runner async, corridos en paralelo por `runAllAgents` (con callback de progreso por agente, base del WebSocket):
  ...
  - **`soil`**: **real**, SoilGrids (ISRIC, sin auth) con cache de 24h y fallback a mock si el fetch falla o si el punto no tiene cobertura SoilGrids. **Corrección post-revisión final (2026-09-12)**: el fetch real tenía tres defectos que causaban un fallback a mock en *toda* consulta, no solo cuando no había cobertura genuina: (1) `depth: '0-30cm'` no es un intervalo válido para `phh2o/soc/clay/sand` — SoilGrids devuelve `layers: []` vacío con ese parámetro; corregido pidiendo los tres intervalos superficiales válidos (`0-5cm`, `5-15cm`, `15-30cm`, repetidos vía `params.append('depth', ...)`) y promediando sus medias; (2) la respuesta real anida los valores un nivel más profundo — `layer.depths[].values.mean`, no `layer.values.mean` como asumía el código — lo que producía un `TypeError` silencioso capturado por el `catch`; (3) cada media viene escalada por `layer.unit_measure.d_factor` (p. ej. pH mapeado como pH×10) y debe dividirse para obtener el valor real. Los tres se corrigieron en `server/agents/soil/soilAgent.ts` (tipo `SoilGridsResponse`, construcción de `params`, y `meanOf`); fixtures de test actualizados a la forma anidada real. Ver verificación en vivo más abajo.
...
- Con `npm run dev:all` corriendo (Vite :5173 + API :3001), navegador contra `http://localhost:5173`:
  ...
  - `agents.soil.sources[0].provider` = `"Mock"` **(diagnóstico corregido el 2026-09-12)**: en esta corrida original se documentó como "SoilGrids no tuvo cobertura para el punto del lote" — ese diagnóstico era incorrecto. La causa real era el bug de parseo descrito arriba (parámetro `depth` inválido → `layers` vacío → excepción → mock), que se disparaba en *cualquier* coordenada, no solo esta. Ya corregido; verificación en vivo post-fix: una consulta real a `https://rest.isric.org/soilgrids/v2.0/properties/query` con los parámetros corregidos para `lat=-32.69&lon=-62.10` (el punto del lote de prueba) devolvió las 3 profundidades válidas pero `mean: null` en las cuatro propiedades para ese punto exacto (corte de cobertura genuino, o ISRIC con datos incompletos ese día). Para confirmar que el parseo en sí funciona contra una respuesta real no nula, se repitió la misma consulta en un punto con cobertura conocida (Países Bajos, `lat=52.0&lon=5.5`) y se obtuvieron valores reales: `phh2o` promedio 47.67 (÷ `d_factor` 10 = pH 4.77), `clay` 243.33 (÷10 = 24.3%), `sand` 537.33 (÷10 = 53.7%), `soc` 738.33 (÷10 = 73.8 g/kg) — las cuatro capas con `d_factor: 10`. El fallback a mock sigue siendo un comportamiento válido y esperado si ISRIC está caído, rate-limitado, o genuinamente no tiene cobertura para un punto — pero ya no es lo que estaba ocurriendo en esta corrida.
